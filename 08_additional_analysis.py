...
def compute_aov_roas_ci(abt):
    ...
    plt.xlabel('Mean AOV')
    plt.tight_layout()
    plt.savefig('outputs/charts/bootstrap_aov.png', dpi=150)
    plt.close()
    ...
    if roas_boot.size:
        ...
        plt.tight_layout()
        plt.savefig('outputs/charts/bootstrap_roas.png', dpi=150)
        plt.close()
...
def kaplan_meier_retention():
    ...
    plt.tight_layout()
    plt.savefig('outputs/charts/kaplan_km.png', dpi=150)
    plt.close()
    ...
    try:
        ...
        plt.ylabel('Survival')
        plt.tight_layout()
        plt.savefig('outputs/charts/kaplan_by_segment.png', dpi=150)
        plt.close()
    except Exception:
        pass
...
def clean_campaigns_and_margin_roas(margin_rate=0.30):
    ...
    plt.tight_layout()
    plt.savefig('outputs/charts/campaign_spend_roas.png', dpi=150)
    plt.close()
...
def main():
    ...
    print('Additional analysis complete. Outputs written to outputs/additional_analysis.md and outputs/charts/*.png')
